...
from api_dtos import feedback_input, profile_input, project_input, technology_input
from database import listar_clientes, adicionar_cliente, listar_ordens, adicionar_ordem, buscar_cliente_por_id, buscar_ordem_por_id, buscar_itens_ordem, atualizar_ordem, excluir_ordem, get_servicos, criar_feedback, criar_profile, buscar_profile, criar_technology, listar_technologies, criar_project, listar_projects, profile_existe, technologies_existem
...
def register_routes(app):
    ...
    def api_create_profile():
        try:
            profile = criar_profile(profile_input(json_body()))
    ...
    def api_get_profile(profile_id):
        profile = buscar_profile(profile_id)
        if not profile:
    ...
    def api_create_technology():
        try:
            technology = criar_technology(technology_input(json_body()))
        except ValueError as error:
            return jsonify({'error': str(error)}), 400
        except sqlite3.IntegrityError:
            return jsonify({'error': 'Já existe uma tecnologia com este nome'}), 409
        return jsonify(technology), 201

    @app.route('/api/technologies', methods=['GET'])
    def api_list_technologies():
        return jsonify(listar_technologies())

    @app.route('/api/projects', methods=['POST'])
    def api_create_project():
        try:
            dados = project_input(json_body())
        except ValueError as error:
            return jsonify({'error': str(error)}), 400
        if not profile_existe(dados['profile_id']):
            return jsonify({'error': 'Perfil informado não existe'}), 400
        if not technologies_existem(dados['technology_ids']):
            return jsonify({'error': 'Uma ou mais tecnologias informadas não existem'}), 400
        project = criar_project(dados)
        return jsonify(project), 201

    @app.route('/api/projects', methods=['GET'])
    def api_list_projects():
        return jsonify(listar_projects())

    @app.route('/api/projects/<int:project_id>/feedback', methods=['POST'])
    def api_create_feedback(project_id):
        try:
            dados = feedback_input(json_body())
        except ValueError as error:
            return jsonify({'error': str(error)}), 400
        if not any(project['id'] == project_id for project in listar_projects()):
            return jsonify({'error': 'Projeto não encontrado'}), 404
        return jsonify(criar_feedback(project_id, dados)), 201
